...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for idx in range(0, companies):
            ...
            company.get_order_sum = 0
            company.get_order_count = 0.0
            ...
            for cdx in range(contacts, max_contacts):
                ...
                for odx in range(orders, max_orders):
                    ...
                    order.save()
                # --- The following couple of lines are added
                contact.get_order_count = len(contact.orders.all())
                contact.save()
                # --- end of modification

        #--------- The following section is added to old version of the code
            company.get_order_count = len(company.orders.all())
            total_sum = 0
            for order in company.orders.all():
                total_sum += order.total
            company.get_order_sum = total_sum
            company.save()
        #--------- End of the section
