...
class Company(models.Model):
    ...
    get_order_count = models.IntegerField(null=True)
    get_order_sum = models.DecimalField(max_digits=18, decimal_places=9, null=True)
    ...
    """def get_order_count(self):
        ...
        for order in self.orders.all():
            orders += 1
        return orders
        return len(self.orders.all()) # improvised code.
    ...
    def get_order_sum(self):
        ...
        for order in self.orders.all():     #updated for loop
            total_sum += order.total
        return total_sum"""
